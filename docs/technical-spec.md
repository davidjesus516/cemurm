...
| Zustand (planned) | 4+ | Lightweight state management — not installed today; React context (`useAuth.jsx`) covers current state |
...
Authoritative schema: **implemented in `supabase/migrations/`** — `0001_init.sql` (48 tables, 6 enums; DDL ported verbatim from `docs/database-schema-v2.md` §2) and `0002_rls_core.sql` (RLS: 34 owner-scoped policies, 4 `private`-schema helpers: `is_org_member`, `user_branch_ids`, `session_role_in`, `session_owns_setlist`). The v2 doc is the design origin (approved 2026-09-01, merged via PR #10) and remains the model contract for D1–D7, A1–A8. **Implemented RLS is owner-scoped only** — the full org/branch/system visibility matrix in v2 §3.1 is the design target for later hitos, not yet live. This section is intentionally a pointer, not a duplicate.
...
> `@` = exists today · `~` = planned (design target, not yet in the repo).
